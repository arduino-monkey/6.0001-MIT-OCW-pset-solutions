...
def get_word_score(word, n):
    ...
    if (7 * word_length) - 3*(n - word_length) > 1:
        return total * ((7 * word_length) - 3 * (n - word_length))
    else:
        return total * 1
#
...
def deal_hand(n):
    ...
    for i in range(num_vowels):
        ...
        hand[x] = hand.get(x, 0) + 1 #if letter is not in hand then get fn returns 0 and 1 is added
...
def is_valid_word(word, hand, word_list):
    ...
    """
    word = word.lower()
    if word in word_list or word.replace('*','a') in word_list or word.replace('*','e') in word_list or word.replace('*','i') in word_list or word.replace('*','o') in word_list or word.replace('*','u') in word_list:
        for letter in word:
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while game:
        if calculate_handlen(hand) > 0:
            ...
            if word == '!!':
                print('Total score for this hand:', points)
                print(10*'-')
                return points
                game = False
...
def substitute_hand(hand, letter):
    ...
    while True:
        if chosen_letter in hand.keys():
            chosen_letter = random.choice(string.ascii_lowercase)
        else:
            break
    del(hand[letter])
    hand[chosen_letter] = times
    return hand
...
if __name__ == '__main__':
    ...
    print(f'Total score over all hands: {x}')